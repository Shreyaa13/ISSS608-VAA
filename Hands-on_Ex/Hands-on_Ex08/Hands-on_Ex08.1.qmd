---
title: "Hands-on Ex 8.1 - Chroropleth Mapping with R"
...
Choropleth mapping represents enumeration units—such as countries, provinces, states, counties, or census areas—using area patterns or graduated colors. For instance, a social scientist might use a choropleth map to visualize the spatial distribution of Singapore’s elderly population based on the Master Plan 2014 Subzone Boundary.

In this chapter, we will learn how to create accurate and effective choropleth maps using the [**tmap**](https://cran.r-project.org/web/packages/tmap/) package in R.
...
Among the four packages, **readr**, **tidyr** and **dplyr** are already part of **tidyverse** package. The code chunk below will be used to install and load these packages in RStudio.
...
1.  **Master Plan 2014 Subzone Boundary (Web)** *(MP14_SUBZONE_WEB_PL)* – This is a geospatial dataset in ESRI shapefile format, available for download from [data.gov.sg](https://data.gov.sg/) . It contains the geographical boundaries of Singapore at the planning subzone level, based on the URA Master Plan 2014.

2.  **Singapore Residents by Planning Area/Subzone, Age Group, Sex, and Type of Dwelling (2011-2020)** – This is an aspatial dataset in CSV format (*respopagesextod2011to2020.csv*), available from the [Department of Statistics, Singapore](https://www.singstat.gov.sg/) . While it does not include coordinate values, its PA (Planning Area) and SZ (Subzone) fields serve as unique identifiers, allowing it to be linked to the *MP14_SUBZONE_WEB_PL* shapefile for geocoding.
...
In the following sub-section, we will share with the tmap functions used to plot these elements.
...
-   The default interval binning used to draw the choropleth map is called “pretty”. A detailed discussion of the data classification methods supported by **tmap** will be provided in sub-section 8.4.3.
-   The default colour scheme used is `YlOrRd` of ColorBrewer. We will learn more about the color scheme in sub-section 8.4.4.
